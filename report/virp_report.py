    ...
    if ex["gate"]:
        rows = [[Paragraph("<b>%s</b>" % h, ss["MonoSmall"]) for h in
                 ("session / device", "seq", "timestamp (UTC)",
                  "artifact id", "reason commitment (sha256)")]]
        for v in ex["gate"]:
            e = v.entry
            rows.append([
                ...
                Paragraph(str(e["sequence"]), ss["MonoSmall"]),
                Paragraph(ns_to_utc(e["timestamp_ns"]), ss["MonoSmall"]),
                Paragraph(trunc(e["artifact_id"], 30), ss["MonoSmall"]),
                Paragraph(e["artifact_hash"], ss["MonoSmall"])])
        t = Table(rows, colWidths=(2.3 * inch, 0.4 * inch, 1.35 * inch,
                                   2.1 * inch, 4.15 * inch),
                  hAlign="LEFT", repeatRows=1)
        t.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), HEAD_BG),
        ...
        story.append(Paragraph(
            '<font color="%s"><b>The signed reason text for a gate block is '
            "not recoverable from this chain.</b></font> The daemon writes "
            "only sha256(reason) as the artifact hash and stores no body "
            "(virp_onode.c, \"gatereject-%%.16s\"); the human-readable reason "
            "goes to the daemon journal. The hash above is a commitment: "
            "given the reason text from the journal you can confirm it "
            "matches, but the text cannot be reconstructed from the chain."
            % vhex(verify.UNVERIFIABLE), ss["Note"]))
    else:
    ...
    if ex["red"]:
        ...
        for v in ex["red"]:
            ...
            rows.append([
                ...
                Paragraph(trunc(entry_device(v), 22), ss["MonoSmall"]),
                Paragraph(ns_to_utc(e["timestamp_ns"]), ss["MonoSmall"]),
                Paragraph(trunc(detail, 90), ss["MonoSmall"])])
