...
\textbf{O-Node trust.} VIRP assumes the O-Node is trusted. A compromised O-Node can produce signed fabrications. Hardware trust anchors (TPM) and formal verification of the O-Node binary are planned mitigations. Phase~1 of the formal verification effort (protocol-level, not binary-level) is already complete: HMAC key secrecy and non-injective agreement are machine-verified in ProVerif and Tamarin under a Dolev--Yao attacker \cite{draft-howard-virp-05}. Injective agreement and counter uniqueness remain open.
...
\textbf{Formal verification (Phase 2).} Phase~1 machine-verified HMAC key secrecy and non-injective agreement in ProVerif and Tamarin under Dolev--Yao \cite{draft-howard-virp-05}. Phase~2 targets the two properties that remain open---injective agreement and counter uniqueness---and extends the model to channel-key binding and tier enforcement logic.
