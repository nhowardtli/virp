...
\textbf{O-Node trust.} VIRP assumes the O-Node is trusted. A compromised O-Node can produce signed fabrications. Hardware trust anchors (TPM) and formal verification of the O-Node binary are planned mitigations. At the protocol level (not binary-level): (1)~injective agreement between v2 observation acceptance and signing, plus master-key and session-key secrecy, are machine-verified in ProVerif~2.05 under a Dolev--Yao attacker; the model and raw output are checked into the reference implementation at \texttt{proofs/virp\_obs\_v2.pv} and \texttt{proofs/virp\_obs\_v2.out}, re-runnable via \texttt{make proofs}. (2)~The proof holds under a stated trace restriction matching \texttt{virp\_seqstore\_accept()}, whose correctness is demonstrated by the replay negative tests in \texttt{tests/test\_obs\_v2.c}, including persistence across verifier restart. (3)~Timestamp freshness is test-verified only, not modeled. There is no Tamarin model; the legacy v1 message path is not covered by the proofs.
...
\textbf{Formal verification (Phase 2).} The checked-in ProVerif~2.05 model covers key secrecy and injective agreement for the v2 observation path, under the stated replay-counter restriction (see \texttt{proofs/} in the reference implementation). Phase~2 targets a stateful-verifier model (Tamarin or StatVerif — future work) that discharges the replay-counter restriction inside the model itself, adds the freshness window, and extends coverage to channel-key binding and tier enforcement logic.
...
\bibitem{draft-howard-virp-05}
N. Howard,
``VIRP: Verified Infrastructure Response Protocol,''
Internet-Draft draft-howard-virp-05, IETF, work in progress, 2026.
