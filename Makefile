...
$(TEST_CISCO_IO): tests/test_driver_cisco_io.c $(LIB)
	@ar t $(LIB) 2>/dev/null | grep -q '^virp_ssh_hostkey.o$$' || { \
	  echo "ERROR: $@ needs virp_ssh_hostkey.o (virp_ssh_verify_hostkey) in"; \
	  echo "       $(LIB), which is only built when an SSH driver flag is set"; \
	  echo "       (CISCO/FORTIGATE/PANOS/ASA/JUNIPER/LINUX). Run e.g."; \
	  echo "       'make CISCO=1 $@'."; \
	  false; }
	rm -f $@
	$(CC) $(CFLAGS) $< $(LIB) $(LDFLAGS) -lssh2 -o $@

test-cisco-io: $(TEST_CISCO_IO)
	./$(TEST_CISCO_IO)

# Typed config op: interface <name> description <text> (C-04 Option A on the
# C node, 2026-09-15). Parser, classifier agreement, and the refusals that
# must happen before any transport byte. Includes driver_cisco.c directly,
# like the refusal test, so it needs the same SSH-driver build flags.
TEST_CISCO_DESCRIPTION = $(BUILD_DIR)/test_driver_cisco_description
$(TEST_CISCO_DESCRIPTION): tests/test_driver_cisco_description.c tests/refusal_contract.h $(LIB)
	rm -f $@
	$(CC) $(CFLAGS) $< $(LIB) $(LDFLAGS) -o $@

.PHONY: test-cisco-description
test-cisco-description: $(TEST_CISCO_DESCRIPTION)
	./$(TEST_CISCO_DESCRIPTION)
...
.PHONY: test-tacacs-ham
test-tacacs-ham:
	@echo "=== TACACS+ HAM review regressions (2026-09-06) ==="
	python3 tests/test_tacacs_ham.py
...
.PHONY: test-health-uid-passthrough
test-health-uid-passthrough:
	@echo "=== ONODE_ACTION_HEALTH carries the client uid into the tier gate ==="
	python3 tests/test_health_uid_passthrough.py
...
test-drivers:
	...
	$(MAKE) BUILD_DIR=$(DRIVER_BUILD_DIR) CISCO=1 PANOS=1 ASA=1 JUNIPER=1 FORTIGATE=1 LINUX=1 WAZUH=1 LIBRENMS=1 PBS=1 ZAMMAD=1 \
	        ...
	        test-pbs test-pbs-gate test-zammad-gate test-typed-hash test-ingress-nul test-pbs-trunc test-cisco-description
...
VIRP_SHELL_RED_UID    = 984
VIRP_SHELL_RED_USER   = virp-shell-red
...
install-virp-shell:
	...
	@id -u $(VIRP_SHELL_RED_USER) >/dev/null 2>&1 || { \
	    echo "FAIL: user $(VIRP_SHELL_RED_USER) does not exist (the 'enable red' seat). Create it first:"; \
	    echo "  useradd --system --uid $(VIRP_SHELL_RED_UID) -g virp --shell /usr/sbin/nologin $(VIRP_SHELL_RED_USER)"; \
	    exit 1; }
	@[ "$$(id -u $(VIRP_SHELL_RED_USER))" = "$(VIRP_SHELL_RED_UID)" ] || { \
	    echo "FAIL: $(VIRP_SHELL_RED_USER) is uid $$(id -u $(VIRP_SHELL_RED_USER)), not $(VIRP_SHELL_RED_UID)"; exit 1; }
	@[ "$$(id -gn $(VIRP_SHELL_RED_USER))" = "virp" ] || { \
	    echo "FAIL: $(VIRP_SHELL_RED_USER)'s primary group is $$(id -gn $(VIRP_SHELL_RED_USER)), not virp"; exit 1; }
