...
/* Item 5 (2026-08-11): the scrub GROWS the body (each redaction can
 * add bytes), so a config that fit the capture buffer before
 * scrubbing can exceed it after. output_len used to store the length
 * snprintf WOULD have written — a clamped body signed with
 * success=true and truncated=no. cisco_store_output must never
 * report a clamped body as complete. */

/* A config just under VIRP_OUTPUT_MAX pre-scrub that grows well past
 * it post-scrub: "password 1\n" (11 B) redacts to
 * "password <removed>\n" (19 B). */
...
TEST(test_scrub_growth_overflow_withheld)
{
    ...
    size_t slen = 0;
    CHECK(cisco_scrub_config(GROW_CFG, cfg_len, GROW_SCRUBBED,
...
{
    /* Same oversized body on the NON-scrub path: the legacy clamped
     * copy is kept, but output_len must be the ACTUAL stored length
     * (fortigate precedent) and output_truncated must be set — never
     * signed as truncated=no. */
    build_grow_cfg();
    size_t slen = 0;
    cisco_scrub_config(GROW_CFG, strlen(GROW_CFG), GROW_SCRUBBED,
    ...
    CHECK(res.output_truncated,
          "clamped body reported truncated=no");
