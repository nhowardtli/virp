 ...
 *
 * Iterates ASA's OWN classification table and asserts, for every
 * entry, that asa_route_command(entry) returns the tier that entry declares.
 *
...
static void test_table_driven_all_entries(void)
{
    ...
    size_t total = ASA_ROUTE_TABLE_SIZE;
    size_t skipped = 0;

    for (size_t i = 0; i < total; i++) {
        const char *cmd = ASA_ROUTE_TABLE[i].command_pattern;
        virp_trust_tier_t declared = ASA_ROUTE_TABLE[i].tier;

        char label[192];
        snprintf(label, sizeof(label), "entry[%zu] %s -> %s",
                 i, cmd, tier_name(declared));
        TEST(label);
        virp_trust_tier_t got = asa_route_command(cmd);
        if (got != declared) {
