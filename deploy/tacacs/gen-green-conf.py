...
# BLACK included, 2026-09-08. It was omitted, so check_no_green_shadows_worse
# could not see BLACK rows at all and would not have noticed a GREEN row that
# admitted an unapprovable command. It happens to be true for both drivers
# today that no GREEN prefix reaches a BLACK row -- but that was luck, not a
# checked property, and BLACK is the one tier where being wrong is worst.
ROW = re.compile(r'^\s*\{\s*"([^"]+)"\s*,\s*VIRP_TIER_(GREEN|YELLOW|RED|BLACK)\s*\}')
...
def canonicalize(cmds, repo_root):
    ...
    IT IS IOS-SPECIFIC, AND ON A NON-IOS DRIVER IT IS VACUOUS -- said
    plainly here because it looks like a check either way.  ios_canonical
    validates the VERB against a corpus derived from observed IOS
    accounting and passes unknown later tokens through unchanged.  Every
    ASA GREEN row begins with `show`, which is a known verb, so all 39
    rows pass while nothing ASA-specific about them is actually checked.

    Worse, no ASA corpus can be built the same way: the ASA does not
    account `show` commands at all (finding ASA-SHOW-UNACCOUNTED,
    docs/tacacs/ASA-5525-PHASE0.md), so its accounting stream will never
    contain a single one of these strings.  The equivalent evidence for
    an ASA table is the AUTHORIZATION log on the policy server, which
    does see `show`, and the canonical forms below stay UNVERIFIED
    against the wire until that log has been read.  Pass --no-canonical
    for a non-IOS driver rather than take a pass here for a check.
    """
...
def main():
    ...
    ap.add_argument("--no-canonical", action="store_true",
                    help="skip the IOS canonicalizer -- required for a "
                         "non-IOS driver, where it validates nothing")
    ...
    if args.no_canonical:
        print("NOTE: IOS canonicalization SKIPPED (--no-canonical). The "
              "canonical form of these %d commands is unverified against "
              "the wire; read the authorization log to settle it."
              % len(green), file=sys.stderr)
    else:
    ...
    lines.append("#")
    lines.append("# Source of truth: %s, the GREEN rows of" % args.driver)
    lines.append("# its gate table. Regenerate rather than edit; an edit here")
    ...
    lines.append("# (cmd-arg=<cr>, reassembled by tac_plus-ng author.c). The")
    lines.append("# terminator is OPTIONAL rather than required precisely so")
    lines.append("# this file is correct on a platform that does not send it;")
    lines.append("# whether the ASA does is a measurement, not an assumption,")
    lines.append("# and the authorization log is where it gets settled.")
    lines.append("#")
