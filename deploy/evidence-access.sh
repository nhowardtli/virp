#!/bin/bash
#
# evidence-access.sh — root pre-step for virp-evidence.service.
#
# Grants the `virp-evidence` identity exactly the reach it needs and
# nothing else:
#   - traversal (x only, no read) of /run/virp and /var/lib/virp
#   - rw on the onode socket (unix connect() needs write permission)
#   - ownership of /var/lib/virp/evidence (0700)
#
...
#
# The collector gets NO read on /var/lib/virp/config-backups and the
# backup runbook gets none on /var/lib/virp/evidence: each automation
# owns its own tree, so a compromise of one does not reach the other's
# evidence.
#
...
USER_NAME="virp-evidence"
EVIDENCE_DIR="/var/lib/virp/evidence"
SOCK="/run/virp/onode.sock"

if ! id -u "$USER_NAME" >/dev/null 2>&1; then
    echo "[evidence-access] FATAL: user $USER_NAME does not exist" >&2
...
# the other runbook's group.
extra_groups=$(id -Gn "$USER_NAME" | tr " " "\n" | grep -vx "$USER_NAME" || true)
if [ -n "$extra_groups" ]; then
    echo "[evidence-access] FATAL: $USER_NAME has supplementary" \
         "groups: $extra_groups — it must be a member of no group but" \
         "its own" >&2
    exit 1
fi

install -d -o "$USER_NAME" -g "$USER_NAME" -m 0700 "$EVIDENCE_DIR"
...
if [ ! -S "$SOCK" ]; then
    echo "[evidence-access] FATAL: $SOCK missing — is virp-onode up?" >&2
    exit 1
fi
setfacl -m "u:${USER_NAME}:rw-" "$SOCK"

echo "[evidence-access] $USER_NAME: socket ACL + $EVIDENCE_DIR ready"
