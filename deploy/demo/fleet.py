...
subprocess.run(['install', '-m', '0755', str(source / 'frr-cli.sh'), str(root / 'demo-cli.sh')], check=True)
...
for i in range(1, 5):
    ...
    node['binds'] = [v for v in node['binds'] if '/demo-init.sh' not in v and '/run/demo-password' not in v and '/demo-cli.sh' not in v]
    node['binds'] += [str(root / 'demo-init.sh') + ':/demo-init.sh:ro', str(root / 'demo-cli.sh') + ':/demo-cli.sh:ro', str(secret) + ':/run/demo-password:ro']
